...
namespace RemoteControls
{
    public sealed partial class SerialTerminalPage : Page
    {
        ...
        private async Task ReadAsync(CancellationToken cancellationToken)
        {
            ...
            char delimeter = Commands.ElementConfigCh["cPacketDelimeterChar"];
            string strDelimeter = delimeter.ToString();
            ...
            if (bytesRead > 0)
            {
                try
                {
                    ...
                    if (SerialRecvState.Current == SerialRecvState.States.Start)
                    {
                        while ((this.recvdText.Text.Length>1) && (this.recvdText.Text[0] != delimeter))
                            this.recvdText.Text = this.recvdText.Text.Substring(1);
                        if (this.recvdText.Text == "")
                            return;
                        if (this.recvdText.Text[0] == delimeter)
                        {
                            SerialRecvState.Current = SerialRecvState.States.Recving;
                            if (this.recvdText.Text.Length==1)
                                return;
                            this.recvdText.Text = this.recvdText.Text.Substring(1);
                            if (this.recvdText.Text == "")
                                return;
                    ...
                    if (SerialRecvState.Current == SerialRecvState.States.Recving)
                    {
                        if (this.recvdText.Text.Contains(strDelimeter))
                        {
                            int indx = this.recvdText.Text.IndexOf(strDelimeter);
                            //Remove delimeter on end of string
                            string sendText = this.recvdText.Text.Substring(0, indx );
                            //Send it
                            await Globals.MP.UpdateText(sendText);
                            this.recvdText.Text = this.recvdText.Text.Substring(indx);
                            SerialRecvState.Current = SerialRecvState.States.Start;
                        }
                    }
                    return;
